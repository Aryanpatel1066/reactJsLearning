export default function EmployeeDetails({employees}){
    return(
        ...
        <div> {employees.map(({name,salary,dept,level,designation})=>(
            <li key={level}style={{background:level>2?"orange":"",border:designation === "President"?"2px solid red":""}}>name:{name},level{level},dept:{dept},designation:{designation},salary:{salary}</li>
        ))}</div>
